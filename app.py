...
import datetime
import sqlite3 # Importar para usar SQLite
...
st.markdown(
    ...
    <style>
        ...
        .login-card {
            ...
            background-color: var(--secondary-background-color);
            width: 100%;
            max-width: 300px; /* Ajustado para 300px para centralizar melhor */
            text-align: center;
        }
        .login-card h2 {
            margin-bottom: 20px;
        ...
        .login-card .stTextInput > div > div > input {
            ...
            width: 100%; /* Garante que o input ocupe a largura total do card */
        }
        ...
        /* Estilos para o container principal do cabeçalho (logo + filtros) */
        .main-header-container > div[data-testid="stHorizontalBlock"] {
            display: flex;
        ...
        /* Estilos para o modal de lançamento de execução */
        .stModal {
            position: fixed;
            top: 50%;
            left: 50%;
...
# Matriz hierárquica
# =========================
def montar_matriz_hierarquica(df_base: pd.DataFrame, inicio: pd.Timestamp, qtd_semanas: int = 52, search_term: str = ""):
    leaf_weeks = defaultdict(set)
    search_term_norm = norm(search_term) if search_term else ""

    filtered_df_base = df_base.copy()
    if search_term_norm:
        # Filtra as linhas do DataFrame base que contêm o termo de busca em qualquer uma das colunas de categoria
        mask_search = filtered_df_base.apply(
            lambda row: any(search_term_norm in norm(str(x)) for x in row[[
                "AREA", "EQUIPAMENTO", "CONJUNTO", "SUBCONJUNTO", "PONTO", "SERVICO"
            ]]),
            axis=1
        )
        filtered_df_base = filtered_df_base[mask_search]

    if filtered_df_base.empty and search_term_norm:
        return [] # Retorna vazio se não houver resultados para a busca

    for _, r in filtered_df_base.iterrows():
...
def init_db():
    with get_db_connection() as conn:
        cursor = conn.cursor()
        # Adicionando a coluna 'rota' (que corresponde à 'AREA') e 'periodicidade' à tabela lancamentos
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS lancamentos (
                ...
                rota TEXT,
                periodicidade TEXT, -- Nova coluna para a periodicidade
                timestamp TEXT NOT NULL
            )
        """)
        # Adicionar a coluna 'rota' se ela não existir em uma tabela já criada
        try:
            cursor.execute("ALTER TABLE lancamentos ADD COLUMN rota TEXT")
        except sqlite3.OperationalError as e:
            if "duplicate column name: rota" not in str(e):
                raise e
        # Adicionar a coluna 'periodicidade' se ela não existir
        try:
            cursor.execute("ALTER TABLE lancamentos ADD COLUMN periodicidade TEXT")
        except sqlite3.OperationalError as e:
            if "duplicate column name: periodicidade" not in str(e):
                raise e
        conn.commit()

def insert_lancamento(usuario: str, data: datetime.date, hora_inicio: datetime.time, hora_fim: datetime.time, rota: str, periodicidade: str):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO lancamentos (usuario, data, hora_inicio, hora_fim, rota, periodicidade, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (
                usuario,
                data.isoformat(), # Salva data como string ISO (YYYY-MM-DD)
                hora_inicio.isoformat(), # Salva hora como string ISO (HH:MM:SS)
                hora_fim.isoformat(),
                rota, # Salva a rota/área
                periodicidade, # Salva a periodicidade
                datetime.datetime.now().isoformat() # Salva timestamp completo
            )
        )
        conn.commit()

def get_all_lancamentos() -> List[Dict[str, Any]]:
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM lancamentos ORDER BY timestamp DESC")
        lancamentos_db = cursor.fetchall()
        # Converte os resultados para um formato mais amigável (lista de dicionários)
        return [dict(row) for row in lancamentos_db]

def get_unique_users_from_db() -> List[str]:
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT usuario FROM lancamentos ORDER BY usuario ASC")
        users = cursor.fetchall()
        return [row["usuario"] for row in users]

def get_unique_rotas_from_db() -> List[str]:
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT rota FROM lancamentos WHERE rota IS NOT NULL ORDER BY rota ASC")
        rotas = cursor.fetchall()
        return [row["rota"] for row in rotas]

def get_unique_periodicidades_from_db() -> List[str]:
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT periodicidade FROM lancamentos WHERE periodicidade IS NOT NULL ORDER BY periodicidade ASC")
        periodicidades = cursor.fetchall()
        return [row["periodicidade"] for row in periodicidades]
...
def format_time_input(time_str: str) -> str:
    """
    Formata uma string de hora.
    Se for uma sequência de 4 dígitos, insere ':' no meio.
    Ex: "2300" -> "23:00", "830" -> "08:30"
    """
    time_str = time_str.strip()
    if re.fullmatch(r'\d{3,4}', time_str): # Verifica 3 ou 4 dígitos
        if len(time_str) == 3: # Ex: 830 -> 08:30
            time_str = '0' + time_str
...
def show_main_app():
    ...
    candidatos_excel = [
        ...
        "PLANO_DE_LUBRICACAO_V2.xlsx",
        "PLANO DE LUBRICACAO V2.xlsx",
    ...
    if not arquivo_local:
        st.error("Arquivo Excel não encontrado na mesma pasta do app.py.")
        st.stop()
    ...
    with st.container(border=True): # A borda será aplicada aqui, envolvendo tudo
        ...
        with col_filtros:
            ...
            # Nova barra de pesquisa para a categoria, agora no lugar do título "Filtros"
            search_term = st.text_input("", key="category_search_input", placeholder="Pesquisar na Categoria...")

            # Títulos + botões na mesma linha
            # Em mobile, estas colunas se empilharão devido ao CSS @media
            t1, t2, t3, t4 = st.columns([1, 1, 1.2, 1.2], vertical_alignment="center") # Ajustei as proporções para os 4 botões
            with t1:
            ...
            with t3:
                # Botão "Lançar Execução"
                if st.button("Lançar Execução", use_container_width=True, key="btn_lancar_execucao"):
                    st.session_state["show_exec_modal"] = True # Ativa a exibição do modal
                    st.session_state["show_query_modal"] = False # Garante que o outro modal esteja fechado
            with t4:
                # Novo botão "Consultar Lançamentos"
                if st.button("Consultar Lançamentos", use_container_width=True, key="btn_consultar_lancamentos"):
                    st.session_state["show_query_modal"] = True # Ativa a exibição do modal de consulta
                    st.session_state["show_exec_modal"] = False # Garante que o outro modal esteja fechado
    ...
    # =========================
    # Modal de Lançamento de Execução
    # =========================
    exec_modal_placeholder = st.empty()

    if st.session_state.get("show_exec_modal", False):
        with exec_modal_placeholder.container():
            st.markdown('<div class="stModal">', unsafe_allow_html=True)
            st.subheader("Registrar Execução")

            with st.form("execucao_form", clear_on_submit=True):
                data_execucao = st.date_input("Data da Execução", value=datetime.date.today())

                # Obter as opções de 'AREA' do DataFrame principal para o selectbox 'Rota'
                if 'df' in locals() and 'AREA' in df.columns:
                    opcoes_rota = sorted(df['AREA'].dropna().astype(str).unique().tolist())
                else:
                    opcoes_rota = ["N/A"]

                selected_rota = st.selectbox("Rota", options=opcoes_rota, key="exec_rota_select")

                # Nova opção de Periodicidade
                if 'df' in locals() and 'FREQUENCIA' in df.columns:
                    opcoes_periodicidade = sorted(df['FREQUENCIA'].dropna().astype(str).unique().tolist())
                else:
                    opcoes_periodicidade = ["N/A"]

                selected_periodicidade = st.selectbox("Periodicidade", options=opcoes_periodicidade, key="exec_periodicidade_select")


                current_time_str = datetime.datetime.now().strftime("%H:%M")
                one_hour_later_str = (datetime.datetime.now() + datetime.timedelta(hours=1)).strftime("%H:%M")

                if "hora_inicio_input" not in st.session_state:
                    st.session_state["hora_inicio_input"] = current_time_str
                if "hora_fim_input" not in st.session_state:
                    st.session_state["hora_fim_input"] = one_hour_later_str

                hora_inicio_str_raw = st.text_input("Hora de Início (HH:MM)", value=st.session_state["hora_inicio_input"], max_chars=5, help="Formato: HH:MM (ex: 08:30 ou 830)")
                hora_fim_str_raw = st.text_input("Hora de Fim (HH:MM)", value=st.session_state["hora_fim_input"], max_chars=5, help="Formato: HH:MM (ex: 09:30 ou 930)")

                col_submit_form, col_cancel_form = st.columns(2) # Duas colunas para os botões

                with col_submit_form:
                    submitted = st.form_submit_button("Salvar Lançamento", use_container_width=True)
                with col_cancel_form:
                    if st.form_submit_button("Cancelar", use_container_width=True, type="secondary"): # Botão de cancelar ao lado
                        st.session_state["show_exec_modal"] = False
                        if "hora_inicio_input" in st.session_state:
                            del st.session_state["hora_inicio_input"]
                        if "hora_fim_input" in st.session_state:
                            del st.session_state["hora_fim_input"]
                        st.rerun()

                if submitted:
                    hora_inicio_str_formatted = format_time_input(hora_inicio_str_raw)
                    hora_fim_str_formatted = format_time_input(hora_fim_str_raw)

                    st.session_state["hora_inicio_input"] = hora_inicio_str_formatted
                    st.session_state["hora_fim_input"] = hora_fim_str_formatted

                    valid_time_format = re.compile(r"^(?:2[0-3]|[01]?[0-9]):(?:[0-5]?[0-9])$")

                    hora_inicio_obj = None
                    hora_fim_obj = None
                    validation_error = False

                    if not valid_time_format.match(hora_inicio_str_formatted):
                        st.error(f"Formato da Hora de Início inválido: '{hora_inicio_str_formatted}'. Use HH:MM.")
                        validation_error = True
                    else:
                        try:
                            h_i, m_i = map(int, hora_inicio_str_formatted.split(':'))
                            hora_inicio_obj = datetime.time(h_i, m_i)
                        except ValueError:
                            st.error(f"Hora de Início inválida: '{hora_inicio_str_formatted}'. Verifique os valores de hora e minuto.")
                            validation_error = True

                    if not valid_time_format.match(hora_fim_str_formatted):
                        st.error(f"Formato da Hora de Fim inválido: '{hora_fim_str_formatted}'. Use HH:MM.")
                        validation_error = True
                    else:
                        try:
                            h_f, m_f = map(int, hora_fim_str_formatted.split(':'))
                            hora_fim_obj = datetime.time(h_f, m_f)
                        except ValueError:
                            st.error(f"Hora de Fim inválida: '{hora_fim_str_formatted}'. Verifique os valores de hora e minuto.")
                            validation_error = True

                    if not validation_error and hora_inicio_obj and hora_fim_obj:
                        ...
                        insert_lancamento(
                            ...
                            rota=selected_rota, # Passa a rota selecionada
                            periodicidade=selected_periodicidade # Passa a periodicidade selecionada
                        )
                        st.success(f"Lançamento salvo! Data: {data_execucao}, Rota: {selected_rota}, Periodicidade: {selected_periodicidade}, Início: {hora_inicio_obj.strftime('%H:%M')}, Fim: {hora_fim_obj.strftime('%H:%M')}")
                        st.session_state["show_exec_modal"] = False
                        if "hora_inicio_input" in st.session_state:
                            del st.session_state["hora_inicio_input"]
                        if "hora_fim_input" in st.session_state:
    ...
    # =========================
    # Modal de Consulta de Lançamentos
    # =========================
    query_modal_placeholder = st.empty()

    if st.session_state.get("show_query_modal", False):
        with query_modal_placeholder.container():
            st.markdown('<div class="stQueryModal">', unsafe_allow_html=True)
            st.subheader("Consultar Lançamentos")

            # Obter lista de usuários únicos do banco de dados
            all_users = get_unique_users_from_db()
            all_users.insert(0, "Todos") # Adiciona opção para todos os usuários

            # Obter lista de rotas únicas do banco de dados
            all_rotas = get_unique_rotas_from_db()
            all_rotas.insert(0, "Todas") # Adiciona opção para todas as rotas

            # Obter lista de periodicidades únicas do banco de dados
            all_periodicidades = get_unique_periodicidades_from_db()
            all_periodicidades.insert(0, "Todas") # Adiciona opção para todas as periodicidades

            with st.form("consulta_form", clear_on_submit=False):
                col_user, col_rota, col_periodicidade = st.columns(3)
                with col_user:
                    selected_user = st.selectbox("Filtrar por Usuário", options=all_users, key="query_user_filter")
                with col_rota:
                    selected_rota_query = st.selectbox("Filtrar por Rota", options=all_rotas, key="query_rota_filter")
                with col_periodicidade:
                    selected_periodicidade_query = st.selectbox("Filtrar por Periodicidade", options=all_periodicidades, key="query_periodicidade_filter")


                col_date_start, col_date_end = st.columns(2)
                with col_date_start:
                    date_start = st.date_input("Data Início", value=datetime.date.today() - datetime.timedelta(days=30), key="query_date_start")
                with col_date_end:
                    date_end = st.date_input("Data Fim", value=datetime.date.today(), key="query_date_end")

                submitted_query = st.form_submit_button("Buscar Lançamentos")

            if submitted_query:
                # Buscar todos os lançamentos do DB
                all_lancamentos_db = get_all_lancamentos()
                filtered_lancamentos = []

                for lancamento_db in all_lancamentos_db:
                    # Converte strings de data/hora de volta para objetos datetime
                    lancamento_data = datetime.date.fromisoformat(lancamento_db["data"])
                    lancamento_hora_inicio = datetime.time.fromisoformat(lancamento_db["hora_inicio"])
                    lancamento_hora_fim = datetime.time.fromisoformat(lancamento_db["hora_fim"])
                    lancamento_timestamp = datetime.datetime.fromisoformat(lancamento_db["timestamp"])
                    lancamento_rota = lancamento_db.get("rota", "N/A") # Pega a rota, com fallback para "N/A"
                    lancamento_periodicidade = lancamento_db.get("periodicidade", "N/A") # Pega a periodicidade, com fallback para "N/A"

                    # Filtrar por usuário
                    if selected_user != "Todos" and lancamento_db["usuario"] != selected_user:
                        continue
                    # Filtrar por rota
                    if selected_rota_query != "Todas" and lancamento_rota != selected_rota_query:
                        continue
                    # Filtrar por periodicidade
                    if selected_periodicidade_query != "Todas" and lancamento_periodicidade != selected_periodicidade_query:
                        continue
                    # Filtrar por intervalo de datas
                    if not (date_start <= lancamento_data <= date_end):
                        continue

                    filtered_lancamentos.append({
                        "Usuário": lancamento_db["usuario"],
                        "Rota": lancamento_rota, # Adiciona a rota na exibição
                        "Periodicidade": lancamento_periodicidade, # Adiciona a periodicidade na exibição
                        "Data": lancamento_data.strftime("%d/%m/%Y"),
                        "Hora Início": lancamento_hora_inicio.strftime("%H:%M"),
                        "Hora Fim": lancamento_hora_fim.strftime("%H:%M"),
                        "Registro": lancamento_timestamp.strftime("%d/%m/%Y %H:%M:%S")
                    })

                if filtered_lancamentos:
                    df_lancamentos = pd.DataFrame(filtered_lancamentos)
                    st.dataframe(df_lancamentos, use_container_width=True)
                else:
                    st.info("Nenhum lançamento encontrado com os filtros aplicados.")

            if st.button("Fechar Consulta", key="close_query_modal", use_container_width=True):
    ...
    if df_f.empty and not search_term: # Se não houver dados e não houver termo de busca, exibe aviso
    ...
    # Matriz
    # =========================
    # Passa o termo de busca para a função que monta a matriz
    rows = montar_matriz_hierarquica(df_f, inicio, qtd_semanas=52, search_term=search_term)

    if not rows and search_term: # Se não houver resultados para a busca
        st.info(f"Nenhum resultado encontrado para '{search_term}' nas categorias.")
        # Não st.stop() aqui para permitir que o usuário limpe a busca
    elif not rows: # Se não houver dados para os filtros (sem busca)
        st.warning("Não foi possível montar a matriz hierárquica com os filtros selecionados.")
        st.stop()
...
# =========================
# Inicializa o banco de dados na primeira execução do script
init_db()
